...
public class FCS {
	...
	private static final String PATH = "src/main/resources/com/bo/data/DB.xlsx";
	...
	public User userInfo(String phone) {
		...
		try {
			...
			while (recordset.next()) {
				...
				response = new User(recordset.getField("PHONE"), recordset.getField("STATUS"),
	...
	public void addNewUser(User user) {
		...
		String strAddUser = "INSERT INTO CONTACTOS(PHONE,STATUS,IS_QUEUED) VALUES('" + user.getPhone() + "','"
				+ user.getStatus() + "','true')";
		try {
	...
	public List<User> getRequest() {
		...
		try {
			...
			while (recordset.next()) {
				System.out.println("STATUS " + recordset.getField("STATUS") + " PHONE " + recordset.getField("PHONE"));
				User user = new User(recordset.getField("PHONE"), recordset.getField("STATUS"),
						Boolean.parseBoolean(recordset.getField("IS_QUEUED")), true);
				response.add(user);
			}
			recordset.close();
